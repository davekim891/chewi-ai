...
- Hero interactions: the plate tilts with the pointer; hovering a surface lights its tag; clicking or tapping a surface or tag opens a detail card (kind, id, role, joint, a one-line note from `KIND_NOTES` in `js/bike.js`); Escape or the close button clears it. With the turntable clip present, dragging scrubs the rotation.
