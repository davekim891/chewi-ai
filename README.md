...
- Hero: `js/hero-mesh.js` loads `assets/bike.glb`, a copy of the Meshy v3 bike (Meshy geometry with the approved render projected on and action surfaces painted; see `3d/v3/README.md`). It renders as a textured hologram on the engine and opens in the render's own pose; the camera and the ten anchor positions come from `3d/v3/view_v3.json` (computed by `tools/match-view.html`). The camera radius now fits the scene bounds to the stage with a margin, so the whole bike stays inside at every orbit angle and viewport. The previous TRELLIS-2 reconstruction moved to `3d/trellis/`. Drag to orbit, hover to light a surface, click to open the detail card and highlight the manifest row. No WebGL means the render image instead. `tools/mesh-anchors.html` and `tools/glb-preview.html` are inspection helpers; `js/hero-image.js` (render with live tags, no 3D) is kept as an alternative hero.
- Hologram engine: `js/holo.js` (three.js loading, fallbacks, resize, visibility, drag, hover, labels, status hook), shared by three scenes.
