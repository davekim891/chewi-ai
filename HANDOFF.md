...
4. **Current:** `js/hero-mesh.js` loads `assets/bike.glb`, a copy of the Meshy v3 bike (Meshy geometry with the approved render projected on and action surfaces painted; see `3d/v3/README.md`). Draws it as a textured hologram on the engine, opens in the render's pose. Camera and the ten anchors come from `3d/v3/view_v3.json` (`tools/match-view.html` against the v3 mesh). The TRELLIS-2 reconstruction moved to `3d/trellis/`. Drag, hover, click-to-inspect (detail card + manifest row highlight) all verified in Chrome.
