...
- Hero: `js/hero-mesh.js` loads `assets/bike.glb`, a copy of the Meshy v3 bike (Meshy geometry with the approved render projected on and action surfaces painted; see `3d/v3/README.md`). It renders as a textured hologram on the engine and opens in the render's own pose; the camera and the ten anchor positions come from `3d/v3/view_v3.json` (computed by `tools/match-view.html`). The camera radius is fitted to the stage: one static radius keeps the whole bike (sampled mesh points plus the two tyre floor glows) inside a 4 % inset at every angle of the untouched auto-orbit on any viewport, and a drag past that envelope dollies the camera back just far enough, easing in again when released. The previous TRELLIS-2 reconstruction moved to `3d/trellis/`. Drag to orbit, hover to light a surface, click to open the detail card. No WebGL means the render image instead. `tools/mesh-anchors.html` and `tools/glb-preview.html` are inspection helpers; `js/hero-image.js` (render with live tags, no 3D) is kept as an alternative hero.
- Hologram engine: `js/holo.js` (three.js loading, fallbacks, resize, visibility, drag, hover, labels, status hook), shared by three scenes.
...
- Hero interactions: the plate tilts with the pointer; hovering a surface lights its tag; clicking or tapping a surface or tag opens a detail card (kind, id, role, a one-line note from `KIND_NOTES` in `js/bike.js`); Escape or the close button clears it. With the turntable clip present, dragging scrubs the rotation.
